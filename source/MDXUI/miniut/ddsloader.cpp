...
int BitsPerPixel(_In_ DXGI_FORMAT fmt);
...
HRESULT LoadTextureDataFromFile(const wchar_t* fileName, std::unique_ptr<uint8_t[]>& ddsData, DDS_HEADER** header, BYTE** bitData, int* bitSize);
...
HRESULT LoadTextureDataFromFile(const wchar_t* fileName, std::unique_ptr<uint8_t[]>& ddsData, DDS_HEADER** header, BYTE** bitData, int* bitSize)
{
	...
	ddsData.reset(new (std::nothrow) uint8_t[(int)filesize]);
	if (!ddsData)
	{
		hr = E_OUTOFMEMORY;
		goto DONE;
	...
	// read the data in
	int BytesRead = (int)fread(ddsData.get(), 1, (int)filesize, f);
	...
	*bitSize = (int)filesize - (int)offset;
